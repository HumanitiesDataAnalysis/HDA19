...
model = read.vectors("~/Dropbox/RMP.bin")
```
...
model[[unprofessional_words, average=F]] %>% 
  plot(method="pca")
```
...
```{r}
model2 = read.vectors("glove.bin")
history_words = model2 %>% closest_to(~"history", n=50) %>% pull(word)
model2[[history_words, average=F]] %>% 
  plot(method="pca")
```
...
```{r}
all_history = model %>% closest_to(~ "history",n=Inf, fancy_names=F)
all_history_model2 = model2 %>% closest_to(~ "history", n = Inf,fancy_names=F)
...
model %>% closest_to(~"dude" - "man" + "woman")
...
list1 = model %>% closest_to(gender, n=Inf, fancy_names=F)
list2 = model %>% closest_to(quality, n=Inf, fancy_names=F)
